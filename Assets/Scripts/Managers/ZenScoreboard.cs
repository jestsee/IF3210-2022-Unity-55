...
public class ZenScoreboard : MonoBehaviour
{
    ...
    private void CreateScoreEntryTransform(ScoreEntry scoreEntry, Transform container, List<Transform> transformList)
    {
        ...
        float time = scoreEntry.time;
        entryTransform.Find("Time").GetComponent<Text>().text = time.ToString(); //ke scoreboard
        ...
        if (rank == 1)
        {
            ...
            entryTransform.Find("Time").GetComponent<Text>().color = colorFirst;
    ...
    public void AddScoreEntry(/*int score, int wave,*/float time, string name)
    {
        // create scoreEntry
        ScoreEntry scoreEntry = new ScoreEntry { name = name, time = time/*score = score, wave = wave */};

        // load saved scores
        string jsonString = PlayerPrefs.GetString("scoreTableZen");
        ...
        PlayerPrefs.SetString("scoreTableZen", json);
        PlayerPrefs.Save();
    ...
    {
        PlayerPrefs.DeleteKey("scoreTableZen");
